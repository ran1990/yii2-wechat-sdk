...
abstract class BaseWechat extends Component
{
    ...
    public function getAccessToken($force = false)
    {
        ...
        if ($this->token_db) {
            $this->getDbAccessToken($force);
        }else {
            $this->getFileAccessToken($force);
        }
     ...
     * @param $force 是否强制获取
     */
    private function getFileAccessToken($force = false) {
     ...
     * @param $force 是否强制获取
     */
    private function getDbAccessToken($force = false) {
        
        $token_info = Yii::$app->db_weixin->createCommand('select * from weixin.w_token where id =1')->queryOne();
        
        $time = time(); // 为了更精确控制.取当前时间计算
        if ((empty($token_info['token']) || ($token_info['expires'] + 7200) < time())) {
            if (!($result = $this->requestAccessToken())) {
                throw new HttpException(500, 'Fail to get access_token from wechat server.');
            }
            //存储token
            if ($result['access_token']) {
                $result['expire'] = $time;
                $this->trigger(self::EVENT_AFTER_ACCESS_TOKEN_UPDATE, new Event(['data' => $result]));
                Yii::$app->db_weixin->createCommand('update weixin.w_token set `token`="'.$result['access_token'].'",`expires`='.$time.' where id=1')->execute();
            } 
        }else {
            $result['access_token'] =$token_info['token'];
            $result['expire'] = $token_info['expires'];
        }
    ...
    public function getJsApiTicket($force = false)
    {
        ...
        if ($this->token_db) {
            $this->getDbJsApiTicket($force);
        }else {
            $this->getFileJsApiTicket($force);
        }
     ...
     * @param $force 是否强制获取
     */
    private function getDbJsApiTicket($force = false) 
    {
    
        $token_info = Yii::$app->db_weixin->createCommand('select * from weixin.w_token where id =2')->queryOne();
        ...
        if ((empty($token_info['token']) || ($token_info['expires'] + 7200) < time())) {
            ...
            if ($result['ticket']) {
                ...
                Yii::$app->db_weixin->createCommand('update weixin.w_token set `token`="'.$result['ticket'].'",`expires`='.$time.' where id=2')->execute();
            }
        }else {
            $result['ticket'] =$token_info['token'];
            $result['expire'] = $token_info['expires'];
        }
     ...
     * @throws HttpException
     */
    private function getFileJsApiTicket($force = false)
    {
        $time = time(); // 为了更精确控制.取当前时间计算
